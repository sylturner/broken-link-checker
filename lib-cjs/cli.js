...
const argsToOptions = args => {
  ...
  return Object.entries(args).reduce((opts, [argName, argValue]) => {
    if (argName in renames) {
      ...
      opts.requestMethod = _methods.GET_METHOD;
    }
...
/*, finalResult)*/
  ...
  if (result.displayed) {
    ...
    output += (0, _chalk.yellow)((_link$get = link.get(_Link.REBASED_URL)) !== null && _link$get !== void 0 ? _link$get : link.get(_Link.ORIGINAL_URL));
  ...
  checker.on(_events.HTML_EVENT, (tree, robots, response, pageURL) => {
    ...
    if (error != null) {
      ...
      process.exitCode = 1;
        ...
        logPageMetrics();
        logProgress();
        stats.resetPage(); // If nothing after current page
    ...
    if ((0, _supportsSemigraphics.default)()) {
      ...
      gauge.disable();
      keyScanner.release(); // @todo https://github.com/mikaelbr/node-notifier/issues/174
    ...
    console.error(error); // eslint-disable-next-line no-process-exit
...
class Statistics {
  ...
  resetPage() {
    ...
    this.page.results = [];
    this.page.skippedLinks = 0; //this.page.startTime = Date.now();
